...
def inexact_line_search_w(f, x, y, dx, dy, c1=0.1, c2=0.5, beta=0.9):
  ...
  while iter <= max_iter:
        ...
        if f_val <= original_f_val + c1 * y_a * dy:
            if gradient_magnitude[1] * dx >= c2 * original_gradient_magnitude[1] * dy:
                y_a = y_a
            else:
               y_a = beta * y_a
        else :
